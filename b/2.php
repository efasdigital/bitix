    ...
    <script>
      $(document).ready(function() {
        ...
        $("#form").submit(function(event){
            ...
            request.done(function (response, textStatus, jqXHR){
              var data = JSON.parse(response);
                ...
                $('#resultado').html('O '+data.plano+ ' para os <strong>2 beneficiários</strong> custa no total: <strong class="text-success">R$'+data.total+'</strong>');
                $('#rconsulta').show();
            });
        ...
        $( "#selectPlano" ).change(function() {
            if($('#selectPlano').val() == 0 || $('#nome').val() == '' || $('#idade').val() == '' || $('#nome2').val() == '' || $('#idade2').val() == ''){
              $('#invalid').show();
              $('#rconsulta').hide();
            } else {
              $('#invalid').hide();
