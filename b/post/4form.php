...
if($_POST['selectPlano'] == 1){
    $mVidas = 4;
} elseif($_POST['selectPlano'] == 6){
    $mVidas = 2;
} else {
    $mVidas = 1;
}
    ...
    if ($pricePlan[$i]['codigo'] == $_POST['selectPlano'] && $pricePlan[$i]['minimo_vidas'] == $mVidas){
        ...
        for ($u = 0; $u < $countListPlan; $u++){
            if($_POST['selectPlano'] == $getPlan[$u]['codigo']){
                ...
                if ($_POST['idade4'] >= 0 && $_POST['idade4'] <= 17) {
                    $price4 = number_format($pricePlan[$i]['faixa1'], 2, '.', '');
                } else if ($_POST['idade4'] >= 18 && $_POST['idade4'] <= 40) {
                    $price4 = number_format($pricePlan[$i]['faixa2'], 2, '.', '');
                } else {
                    $price4 = number_format($pricePlan[$i]['faixa3'], 2, '.', '');
                }

                $priceTotal = $price1+$price2+$price3+$price4;
